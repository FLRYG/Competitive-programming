...
typedef pair<string,Position> SP;
...
void updateWordSet(vector<vector<char>> &G, unordered_map<string,Position> &ma){
    rep(i,N) rep(j,N){
        string t;
        rep(k,12){
            if(G[i][(j+k)%N]=='.') break;
            t+=G[i][(j+k)%N];
            ma[t]=Position(i,j,i,(j+k)%N);
        }
    }
    rep(i,N) rep(j,N){
        string t;
...
Ginfo solve(vector<vector<char>> &ans){
    ...
    unordered_map<string,Position> now_words;
    updateWordSet(ans,now_words);
    ...
    while(TIMER.getTime()<=TIME_LIMIT){
        ...
        // rep(iii,100){
            ...
            unordered_map<string,Position> tmp_words;
            updateWordSet(G,tmp_words);

            rep(i,M){
                if(tmp_S_pos[i].a==-1){
                    tmp_S_pos[i]=tmp_words[S[i]];
